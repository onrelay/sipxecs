...
app_exit_code OSSApp::_exitCode = OSS::APP_EXIT_OK;
...
void OSSApp::initialize(OSSApplication& self)
{
  ...
  #if OSS_OS_FAMILY_WINDOWS
    fn = OSS::string_left(fn, fn.size() - 4);
  #endif
...
{
  OSS_VERIFY(OSSApp::_mainHandler);
  OSS_VERIFY(!OSSApp::_isRunning);
  OSSApp app;
  app._argc = argc;
  app._argv = argv;
...
{
  OSS_VERIFY_NULL(OSSApp::_pLogger);
...
{
  OSS_VERIFY_NULL(OSSApp::_pConfig);
