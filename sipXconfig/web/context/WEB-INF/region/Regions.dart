...
class RegionEditor {
  ...
  edit(Map<String, Object> region) {
    ...
    List<String> addresses = region['addresses']! as List<String>;
  ...
  List<String> addresses() {
    List<String> addresses = [];
    if( ipaddr0().value != null && ipaddr0().value!.trim().length > 0 ) {
      addresses.add( ipaddr0().value! );
    }
...
class ManageRegions {
    ...
    if (event != null) {
      ...
      if (req.status != 200) {
        if( req.responseText != null ) {
          var err = jsonDecode(req.responseText!);
          msg.error(err['error']); 
        }           
      }
    }
  ...
  void loadTable(data) {    
    List regions = jsonDecode(data);  
    TableSectionElement tbody = querySelector("#regionTable")! as TableSectionElement;
