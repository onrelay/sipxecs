...
import { log } from "../api/abstractSecurityService";
import { SymmetricCipher } from "../api/symmetricCipher";
import { SymmetricKey } from "../api/symmetricKey";
import { EncryptedRecord } from "../api/encryptedRecord";
...
export class SymmetricCipherImpl implements SymmetricCipher {
    ...
    newInitializationVector() : string {
        ...
        try {
            const initializationVectorLength = configurationServiceFactory!.get().cached(
    ...
    encrypt( data? : any ) : string | undefined {
        ...
        try {
            ...
            const algorithm = configurationServiceFactory!.get().cached(
                "security", "algorithm")!;

            const encoding = configurationServiceFactory!.get().cached(
                "security", "encoding")!; 
