import { Language, LanguageName, Languages } from "@sipxdao/common";
...
import { CountryPropertyImpl } from "../../internal/properties/countryPropertyImpl";
import { DefinitionPropertyImpl } from "../../internal/properties/definitionPropertyImpl";
import { log } from "./abstractDatabaseService";
import { CollectionProperty } from "../properties/collectionProperty";
import { Key, KeysCollectionName } from "./key";
import { CollectionPropertyImpl } from "../../internal/properties/collectionPropertyImpl";
import { KeyFormats, KeyStatus, KeyStatuses, KeyTypes } from "@sipxdao/security";
import { securityServiceFactory } from "@sipxdao/security/src/api/securityServiceFactory";
import { TextProperty } from "../properties/textProperty";
import { TextPropertyImpl } from "../../internal/properties/textPropertyImpl";

export abstract class AbstractEntity extends GenericDatabaseDocument implements Entity {

    constructor( entityDocumentName : string, entityCollection : CollectionDatabase<Entity>, documentPath? : string ) {

        super( entityDocumentName, entityCollection, documentPath ); 

        try {
            this.authenticationId = new TextPropertyImpl( this );
    ...
    async updateKeys() : Promise<boolean> {
        ...
        try {
            ...
            switch( entitySecret.keyStatus.value() )
            {
                ...
                {
                    const symmetricKey = await securityServiceFactory!.get().keyManager?.symmetricKey( this.authenticationId.value()! );
    
                    if( symmetricKey != null && symmetricKey.status !== KeyStatuses.Enabled ) {
    
                        securityServiceFactory!.get().keyManager!.enableSymmetricKey( this.authenticationId.value()! );
                ...
                case KeyStatuses.Disabled:
                {
                    ...
                    if( symmetricKey != null && symmetricKey.status !== KeyStatuses.Disabled ) {
                        ...
                        securityServiceFactory!.get().keyManager!.disableSymmetricKey( this.authenticationId.value()! );
